...
print(f"10Y Par swap rate: {R_swap}")
...
# Problem 4 - Simulation in the CIR Model
np.random.seed(13)
M_simul, T_simul = 1000, 10
size_ci = 0.95
r_simul = fid.simul_cir(r0,a,b,sigma,M_simul,T_simul,method = "milstein")
...
M_simul_cap, N_simul = 2000, 2000
chi = np.zeros([idx_cap_end-1,N_simul])
r_simul_cap = np.zeros([M_simul_cap+1])
for i in range(0,N_simul):
    r_simul_cap = fid.simul_cir(r0,a,b,sigma,M_simul_cap,T_cap_end,method = "milstein")
...
print(f"Caplet prices in bps: {price_caplet*10000}, Cap price in bps: {10000*sum(price_caplet)}")

# Problem 5 - Computing the price of a 2Y3Y swaption
idx_n_swaption, idx_N_swaption = 4, 10
#M_simul_swaption, N_simul, T_simul_swaption = 4000, 20000, T[idx_n_swaption]
M_simul_swaption, N_simul, T_simul_swaption = 2000, 2000, T[idx_n_swaption]
...
R_swaption = 0.045
for i in range(0,N_simul):
    r_simul_swaption = fid.simul_cir(r0,a,b,sigma,M_simul_swaption,T_simul_swaption,method = "milstein")
...
print(f"Swaption price in bps: {10000*price_swaption}")
